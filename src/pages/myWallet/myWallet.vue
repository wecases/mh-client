		...
		<view class="content">
			...
			<view class="recharge ">
				<template v-for="(item,index) in payMoney" :key="index">
					<view @click="actMoney=index+1" :class="{payBut : actMoney!=index+1, act : actMoney==index+1}">{{item}}</view>
				</template>
			</view>
			<view class="custom">
				<text>￥</text>
			<input type="number" placeholder="输入自定义金额">
			</view>
		</view>
		 <view class="box">
...
let nav = ref([
	...
	},
	{
		...
		text:"收益明细",
		jump:"pages/incomeDetail/incomeDetail"
	}
])
// 充值金额
...
.myWallet{
	...
	.topBg{
		background-image: url("../../src/static/img/wallet/myWallet.png");
		width: 100%;
		height: 15.13rem;
